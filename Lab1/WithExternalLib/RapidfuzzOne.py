...
for name1 in only_in_sheet1:
    ...
    for name2 in only_in_sheet2:
        ...
        if score > best_score:  # 找到更高的相似度
            best_score = score
            best_match = name2
    if best_match and best_score > 80:  # 如果相似度大于阈值（例80）
...
# 将结果保存为新的Excel文件
output_path = './OutputsForE/sheet3_with_rapidfuzzOne.xlsx'
with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
    final_output.to_excel(writer, sheet_name='sheet3', index=False)  # 将最终结果写入'sheet3'工作表

# 打印匹配操作所需的时间
print(f"Matching operation time by rapidfuzz: {end_time - start_time:.2f} seconds")
