...
def normalize_journal_name(name):
    name = name.lower().strip()  # 转小写并去除空格
...
# 将结果保存为新的Excel文件
output_path = './OutputsForE/sheet3_with_fuzzywuzzy.xlsx'
with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
    final_output.to_excel(writer, sheet_name='sheet3', index=False)  # 将最终结果写入'sheet3'工作表

# 打印匹配操作所需的时间
print(f"Matching operation time by fuzzywuzzy: {end_time - start_time:.2f} seconds")
