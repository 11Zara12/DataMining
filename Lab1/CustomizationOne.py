...
import time
import re
from EditDistance import levenshtein_ratio  # 引入自定义的编辑距离计算函数
...
# 用线程池并行化编辑距离计算
def find_best_match(name1):
    best_match = None
    best_score = 0  # 初始化最佳匹配的分数
    for name2 in only_in_sheet2:
        score = levenshtein_ratio(name1, name2)  # 使用自定义的Levenshtein比率
        if score > best_score:  # 找到更高的相似度
            best_score = score
            best_match = name2
    if best_match and best_score > 80:  # 如果相似度大于阈值（80）
        return (name1, best_match)
    return None

# 使用线程池进行并行处理
with ThreadPoolExecutor(max_workers=8) as executor:
    matched_names = list(filter(None, executor.map(find_best_match, only_in_sheet1)))
...
# 将结果保存为新的Excel文件
output_path = './sheet3_with_custom_method_optimized.xlsx'
with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
    final_output.to_excel(writer, sheet_name='sheet3', index=False)  # 将最终结果写入'sheet3'工作表

# 打印匹配操作所需的时间
print(f"Matching operation time with custom method (optimized): {end_time - start_time:.2f} seconds")
